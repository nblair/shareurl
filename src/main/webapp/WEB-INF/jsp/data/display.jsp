...
<rs:resourceURL var="flagIcon"
	value="/rs/famfamfam/silk/1.3/flag_blue.png" />
<rs:resourceURL var="taskIcon"
	value="/rs/famfamfam/silk/1.3/table_edit.png" />
...
<c:choose>
	<c:when test="${requestDetails.numberDaysDisplayed > 0 }">
		<c:set var="title"
			value="Agenda for ${startDateFormatted} - ${endDateFormatted}" />
	</c:when>
	<c:otherwise>
...
<c:choose>
	<c:when test="${requestDetails.canonical}">
		<c:url
			value="/u/${shareId}?rss&${requestDetails.canonicalStartEndEncoded}"
			var="rssFeed" />
	</c:when>
	<c:otherwise>
...
<title>${title}</title>
<link rel="alternate" title="${title}" href="${rssFeed}"
...
<a
				...
				<a href="${rssFeed}" title="RSS Feed for this calendar"><img
					src="<c:url value="/img/feed_icon_16x16.gif"/>" /></a>
			</div>
		</div>
							...
							<c:choose>
								<c:when test="${oevent:isDayEvent(event) == true}">
									...
									<span class="weak"><fmt:formatDate
											...
											pattern="MM/dd/yyyy" />&nbsp;All day</span>
								</c:when>
								<c:otherwise>
									...
									<span class="weak"><fmt:formatDate
											...
											value="${event.startDate.date}" type="time" pattern="hh:mm a" />
										- <fmt:formatDate value="${event.endDate.date}" type="time"
											pattern="hh:mm a" /></span>
								</c:otherwise>
							</c:choose>
							<br />
							<fmt:formatDate value="${event.startDate.date}" type="time"
								pattern="yyyy-MM-dd" var="startParam" />
							<fmt:formatDate value="${event.endDate.date}" type="time"
								pattern="yyyy-MM-dd" var="endParam" />
							<c:choose>
								<c:when test="${empty event.recurrenceId}">
									<c:url
										value="/u/${shareId}/${event.uid.value}?start=${startParam}&end=${endParam}"
										var="eventUrl" />
								</c:when>
								<c:otherwise>
									<c:url
										value="/u/${shareId}/${event.uid.value}/${event.recurrenceId.value}?start=${startParam}&end=${endParam}"
										var="eventUrl" />
								</c:otherwise>
							</c:choose>

							<a title="event details" href="${eventUrl}"> <span
								class="summary"> <c:choose>
										<c:when test="${oevent:isCancelled(event)}">
											<del>CANCELLED:&nbsp;${event.summary.value}</del>
										</c:when>
										<c:when test="${event.declinedAttendee}">
											<del>Declined:&nbsp;${event.summary.value}</del>
										</c:when>
